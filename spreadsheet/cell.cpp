...
const char EQUAL='=';
    ...
    public:
        ...
        std::string GetText() const override {
            if (formula_ptr_) {
                return EQUAL + formula_ptr_->GetExpression();
            } else {
                return "";
            }
        }
...
bool Cell::FindCircularDependency(const Impl& new_impl) const {
    ...
    while (!to_visit.empty()) {
        ...
        for (const Cell* incoming : current->left) {
...
void Cell::RenewCache(bool force) {
    if (impl_->IsCacheValid() || force) {
        impl_->InvalidateCache();
        for (Cell* incoming : left) {
            incoming->RenewCache();
        }
    }
}
...
// Деструктор - автоматически освобождает ресурсы
Cell::~Cell() {}
...
void Cell::Set(std::string text) {
    ...
    if (FindCircularDependency(*impl)){
        throw CircularDependencyException("");
    }
    impl_ = std::move(impl);

    for (Cell* outgoing : right) {
        outgoing->left.erase(this);
    }
    ...
    for (const auto& pos : impl_->GetReferencedCells()) {
        ...
        right.insert(outgoing);
        outgoing->left.insert(this);
    }
