...
public:
    ...
    double Evaluate(const SheetArguments& args) const override {
        ...
        double res_value = 0;
        switch (type_) {
            case Add: res_value = lhs_value + rhs_value; break;
            case Subtract: res_value =  lhs_value - rhs_value; break;
            case Multiply: res_value =  lhs_value * rhs_value; break;
            case Divide: res_value =  lhs_value / rhs_value;
        }
        if (!std::isfinite(res_value)) {
